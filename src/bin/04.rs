...
pub fn part_one(input: &str) -> Option<usize> {
    let total: usize = input.lines().map(|line| {
...
pub fn part_two(input: &str) -> Option<u32> {
    let hand_by_card_number: HashMap<u32, CardAndHand> = input.lines().map(|line| {
        let split: Vec<String> = line.split("|").map(|s| s.trim().to_string()).collect();
    ...
    for i in 1..hand_by_card_number.len() + 1 {
        ...
        if !winning_hand.is_empty() {
            ...
            for i in 1..winning_hand.len() + 1 {
                *number_of_copies.entry(card_number + i as u32).or_default() += number_of_current_number_copies;
            }
        }
    }
